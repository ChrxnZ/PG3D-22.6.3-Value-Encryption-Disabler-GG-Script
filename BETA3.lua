...
function START(VALUE_ARG, EDIT_ARG, PAIR_ARG)
    ...
    elseif PAIR_ARG >= MAXP then
    	print("" .. PAIR_ARG .. " Is Higher Than " .. MAXP .. "(Highest Edit Pair Amount Allowed), So Script Will End To Avoid Crashing While Editing High Amount Of Values Per Pair")
    else
        sn(VALUE_ARG .. "X4" ,tpd)
        repeat
            Pair = gr(PAIR_ARG)
...
if Main_MENU == 2 then
    ...
    if config then
        if config[4] then
        	config_1 = tonumber(config[1]) 
        		config_2 = tonumber(config[2]) 
        			config_3 = tonumber(config[3]) 
        				START(config_1, config_2, config_3)
        end
    end
end
